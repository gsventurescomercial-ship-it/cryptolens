...
ai_block = '''function ConversationalAiDialog({ assets, alerts, positions, user, onClose, onNavigate, onRequireAuth }: { assets: MarketAsset[]; alerts: AlertRule[]; positions: PortfolioPosition[]; user: User | null; onClose: () => void; onNavigate: (page: NavItem) => void; onRequireAuth: () => void }) {
...
text, count = re.subn(r"function ConversationalAiDialog\([\s\S]*?\n}\n\nfunction CommandPalette", ai_block + "\n\nfunction CommandPalette", text, count=1)
if count != 1:
    raise SystemExit('ConversationalAiDialog block not found')
...
old_state = """  const [marketOverview, setMarketOverview] = useState<GlobalMarketMetrics | null>(null)
...
new_state = """  const [marketOverview, setMarketOverview] = useState<GlobalMarketMetrics | null>(null)
  const [authUser, setAuthUser] = useState<User | null>(null)
  const [authOpen, setAuthOpen] = useState(false)
  const [alerts, setAlerts] = useState<AlertRule[]>([])
  const [positions, setPositions] = useState<PortfolioPosition[]>([])"""
if old_state not in text:
    raise SystemExit('private state block not found')
text = text.replace(old_state, new_state)

auth_logic = """  const syncPrivateData = useCallback(async (user: User | null) => {
    if (!user) {
      setAlerts([])
      setPositions([])
      return
...
if marker not in text:
    raise SystemExit('hash marker not found')
text = text.replace(marker, auth_logic + marker)
...
if count != 1:
    raise SystemExit('local private storage effects not found')

old_mutations = """  const openAsset = (asset: MarketAsset) => setSelectedAsset(asset)
...
new_mutations = """  const openAsset = (asset: MarketAsset) => setSelectedAsset(asset)
...
text = text.replace("  const toggleWatchlist = (symbol: string) => {", "  const profileSource = authUser?.user_metadata?.name || authUser?.user_metadata?.full_name || authUser?.email || 'Entrar'\n  const profileInitials = profileSource.split(/\\s|@/).filter(Boolean).slice(0, 2).map((part) => part[0]?.toUpperCase()).join('') || 'IN'\n\n  const toggleWatchlist = (symbol: string) => {")
text = text.replace("<button className=\"profile\" onClick={() => showSoon('Perfil')}><span>AM</span><i/></button>", "<button className=\"profile\" onClick={() => setAuthOpen(true)} title={authUser?.email ?? 'Entrar na conta'}><span>{profileInitials}</span><i/></button>")
text = text.replace("onRemoveAlert={(id) => setAlerts((current) => current.filter((item) => item.id !== id))}", "onRemoveAlert={removeAlert}")
text = text.replace("onRemovePosition={(id) => setPositions((current) => current.filter((item) => item.id !== id))}", "onRemovePosition={removePosition}")
text = text.replace("{aiOpen && <ConversationalAiDialog assets={assets} alerts={alerts} positions={positions} onClose={() => setAiOpen(false)} onNavigate={setActiveNav}/>}", "{aiOpen && <ConversationalAiDialog assets={assets} alerts={alerts} positions={positions} user={authUser} onClose={() => setAiOpen(false)} onNavigate={setActiveNav} onRequireAuth={() => setAuthOpen(true)}/>}\n    {authOpen && <AuthDialog user={authUser} onClose={() => setAuthOpen(false)}/>}" )
